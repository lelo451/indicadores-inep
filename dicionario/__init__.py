# -*- coding: utf-8 -*-
"""Engine de parsing e consolidação dos 'DICIONÁRIO DE VARIÁVEIS' do INEP.

Submódulos ``dicionario.enade`` e ``dicionario.idd`` definem as configurações
específicas de cada microdado e chamam ``consolidate(...)`` aqui.

Executar tudo de uma vez: ``python -m dicionario``.
"""
...
def normalize_descricao(variavel: str, value):
    """Pre-normaliza a descricao do dicionário usando as mesmas regras que o
    consolidar_indicadores aplica nas colunas correspondentes."""
    if pd.isna(value):
        return value
    s = str(value).strip()
    if not s:
        return value
    if variavel == "CO_CATEGAD":
        return strip_double_quotes(map_canonical(s, CATEG_ADMIN_CANONICAL))
    if variavel == "CO_ORGACAD":
        return map_canonical(s, ORG_ACAD_CANONICAL)
    if variavel == "CO_GRUPO":
        return drop_engenharia_grupo(s).upper()
    if variavel == "CO_MODALIDADE":
        return s.upper()
    if variavel == "CO_UF_CURSO":
        return clean_uf(s)
    return s
...
def dedupe_distinct_names(long_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = long_df.copy()
    df["descricao"] = [
        normalize_descricao(var, desc)
        for var, desc in zip(df["variavel"], df["descricao"])
    ]
